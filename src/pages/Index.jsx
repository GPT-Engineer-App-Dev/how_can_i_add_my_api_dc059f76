  ...
  const handleSearch = async () => {
    ...
    const searchResults = await Promise.all(
      translatedQueries.map(async (translatedQuery, index) => {
        const googleResponse = await fetch(`https://www.googleapis.com/customsearch/v1?key=${process.env.REACT_APP_GOOGLE_API_KEY}&cx=${process.env.REACT_APP_GOOGLE_SEARCH_ENGINE_ID}&q=${encodeURIComponent(translatedQuery)}`);
